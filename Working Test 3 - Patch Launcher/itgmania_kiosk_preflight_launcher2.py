...
Folder layout (default):
  itgmania_kiosk_preflight_launcher.py
  mods/
...
Notes:
- Everything under mods/simplylove_screenkiosk/Themes/ is treated as patch files and copied
  into the ITGmania folder (same relative path) when needed.
- metrics.ini is patched (surgically) instead of being replaced.

Usage:
  python itgmania_kiosk_preflight_launcher.py "C:\Games\ITGmania"
  python itgmania_kiosk_preflight_launcher.py "C:\Games\ITGmania" --uninstall
  python itgmania_kiosk_preflight_launcher.py "C:\Games\ITGmania" --no-launch
"""
    ...
    existing_entries.append(
        {
            ...
            "changed_utc": datetime.datetime.utcnow().isoformat(timespec="seconds") + "Z",
        }
    )
...
def copy_mod_files_from_source_tree(
    ...
    """
    Copies all files under <mod_source_root>/Themes/ into the ITGmania folder,
    preserving relative paths.
    """
    themes_source_root = mod_source_root_path / "Themes"
    if not themes_source_root.exists():
        raise RuntimeError(f"Missing mod source folder: {themes_source_root}")

    outcomes: List[FileCopyOutcome] = []

    for source_file_path in themes_source_root.rglob("*"):
        ...
        if not destination_file_path.exists():
            raise RuntimeError(f"Copy failed: {destination_file_path}")
...
def install_or_repair_mod(itgmania_root_path: Path, mod_source_root_path: Path, state_root_path: Path) -> None:
    ...
    # Step 2: Copy mod files (all files under mod source Themes/)
