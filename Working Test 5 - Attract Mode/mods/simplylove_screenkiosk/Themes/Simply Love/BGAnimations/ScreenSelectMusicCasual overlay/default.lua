...
---------------------------------------------------------------------------
-- Attract behavior: after a period of inactivity, advance to the next song
-- so its preview audio will play.
local attract_advance_seconds = 60
local attract_check_interval_seconds = 1

local last_activity_time = GetTimeSinceStart()

local function MarkActivity()
	last_activity_time = GetTimeSinceStart()
end

do
	local original_handler = Input.Handler
	Input.Handler = function(event)
		if event and event.type ~= "InputEventType_Release" then
			MarkActivity()
		end
...
local function AdvanceSongPreviewIfPossible()
	if not Input or not Input.Enabled then return end
	if Input.WheelWithFocus ~= SongWheel then return end

	-- Advance once, but skip the CloseThisFolder pseudo-entry so we keep previewing real songs.
	SongWheel:scroll_by_amount(1)
	if SongWheel:get_info_at_focus_pos() == "CloseThisFolder" then
		SongWheel:scroll_by_amount(1)
	end
end
...
local t = Def.ActorFrame {
	...
	Def.Actor{
		Name="AutoAdvancePreview",
		OnCommand=function(self)
			-- Start the idle timer on screen load.
			MarkActivity()
			self:sleep(attract_check_interval_seconds):queuecommand("CheckIdle")
		end,
		CheckIdleCommand=function(self)
			local topscreen = SCREENMAN:GetTopScreen()
			if not topscreen or topscreen:GetName() ~= "ScreenSelectMusicCasual" then return end

			if (GetTimeSinceStart() - last_activity_time) >= attract_advance_seconds then
				AdvanceSongPreviewIfPossible()
				MarkActivity()
			end
